...
def addlabels(x,y, offset=0):
    for i in range(len(x)):
        plt.text(i+offset, y[i], y[i], ha = 'center')
    return

def plot_single_bar(plt, x, y, width, offset=0, color='blue'):
    #TODO
    plt.bar(["{} * {}".format(int(i + offset), 4) for i in x], y, width = width, color=color)
    # configure y
...
def graph_single_algo(graph_info:GraphInfo, width=0.5):
    ...
    for k, v in y.items():
        if last_axis is not None:
            ax = plt.subplot(1, num_plots, count+1, sharex=last_axis)
        else:
            ax = plt.subplot(1, num_plots, count+1)
        last_axis = ax
        plt.ylabel(k, fontsize=16)
        plt.xlabel("{} * thread_num".format(x_label), fontsize=16-num_plots)
