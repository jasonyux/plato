...
class Node2VecTask(Task):
    def __init__(self) -> None:
        super().__init__(os.path.join(SCRIPT_DIR, "measure_node2vec.sh"))
    ...
    def process_performance_info(performance_data:Iterable):
        ...
        for row in performance_data:
            time = re.search("random walk done, cost: (\d+\.\d+)s", row)
            if time is not None:
    ...
    def process_log_group(grouped_data:Iterable):
        ...
        setup_info = Node2VecTask.process_setup_info(grouped_data[0])
        performance_info = Node2VecTask.process_performance_info(grouped_data[1:])
        return {
            "setup": setup_info,
    ...
    def extract_perf(stdout:str):
        ...
        for group in exp_groups:
            start, end = group
            out.append(Node2VecTask.process_log_group(all_data[start:end]))
        return out