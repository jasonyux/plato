...
class LPATask(Task):
    def __init__(self) -> None:
        super().__init__(os.path.join(SCRIPT_DIR, "measure_lpa.sh"))
    ...
    def process_performance_info(performance_data:Iterable):
        ...
        for row in performance_data:
            time = re.search("lpa calculation done: (\d+\.\d+)s", row)
            if time is not None:
    ...
    def process_log_group(grouped_data:Iterable):
        ...
        setup_info = LPATask.process_setup_info(grouped_data[0])
        performance_info = LPATask.process_performance_info(grouped_data[1:])
        return {
            "setup": setup_info,
    ...
    def extract_perf(stdout:str):
        ...
        for group in exp_groups:
            start, end = group
            out.append(LPATask.process_log_group(all_data[start:end]))
        return out