...
OUTPUT="hdfs://100.109.192.161:9000/user/plato/${INCSV}_kcore"
...
run_kcore()
{
    MAIN="$ROOT_DIR/bazel-bin/example/kcore_simple" # process name
...
run_single()
{
    ...
    LOG_FILE="$ROOT_DIR/scripts/log/${INCSV}.kcore.${HOST_NUM}.${KMIN}.${KMAX}.log"
    run_kcore > $LOG_FILE 2>&1
    grep_performance
}
